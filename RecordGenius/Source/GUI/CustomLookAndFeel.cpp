...
namespace GUI
{
                                             ...
                                             juce::Slider &slider)
    {
        ...
        {
            // centre a narrow vertical track
            int trackW = width / 3;
            int trackX = x + (width - trackW) / 2;
            g.setColour(background);
            g.fillRect(trackX, y, trackW, height);

            // filled portion from sliderPos down to bottom
            int fillY = juce::jlimit(y, y + height, (int)sliderPos);
            g.setColour(fillColour);
            g.fillRect(trackX, fillY, trackW, (y + height) - fillY);
        }
        else if (style == juce::Slider::LinearBar) // horizontal bar
        {
            int trackH = height / 3;
            ...
            int fillX = juce::jlimit(x, x + width, (int)sliderPos);
            g.setColour(fillColour);
            g.fillRect(x, trackY, fillX - x, trackH);
        }
        else
        {
            // default JUCE thumb + groove for all other styles
            LookAndFeel_V4::drawLinearSlider(g,
                                             x, y, width, height,
                                             sliderPos,
                                             0.0f, 1.0f, // unused here
                                             style,
                                             slider);
        }
    }
